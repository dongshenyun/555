...
$sql=$db->select(table,"where pid={$pid}");  
...
while($tows=$db->myarray($sqli)){
   ...
   $view=mb_substr($contentes,0,50,'utf-8');
   //$view=cut_str($contentes,200,0,'UTF-8','...');
   echo $url[urlwap]."{$tows['id']}'>{$tows['title']}</a></li></center><br>";
   echo $url[cutwap]."{$tows['id']}'>{$view}......</a></li>";
