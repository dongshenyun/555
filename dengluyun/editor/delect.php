...
$sqlin=$db->select(table,"order by concat(path,id) limit {$fenye->limit()}");
if($sqlin){
while($pows=$db->myarray($sqlin)){
...
echo "<td>{$pows['id']}</td>
           ...
           echo "<td><a onclick=\"return confirm('确定删除么')\" href='action.php?action=del&id={$pows['id']}'>删除</a></td>
		    </tr></center>";
